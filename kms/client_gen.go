...
// Package kms is the KMS API.
//
// Customer-managed encryption keys. Encrypt and decrypt directly, or let
// another service — secrets, storage, images — wrap its data keys
// with one of yours.
//
...
const endpointTemplate = "https://kms.{region}.basaltic.sh"

func init() { basaltic.RegisterServiceEndpoint(ServiceID, endpointTemplate) }

// Client calls the KMS API.
//
...
// New builds a kms client from a shared configuration.
//
